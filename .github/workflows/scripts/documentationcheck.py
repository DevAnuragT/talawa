...
class DocumentationStatus(enum.Enum):
    """Enumeration of documentation statuses."""
    unknown = 0
    updated = 1
    not_updated = 2
    missing = 3
...
def main():
    ...
    try:
        commit_dev = repo.commit(f"origin/{args.merge_branch_name}")
    except Exception:
        commit_dev = repo.commit(args.merge_branch_name)

    # Getting latest commit on feature branch
    feature_commit = repo.commit()
    ...
    if len(filtered_lookup) == 0:
        print(
            "🚀 {} Hurrah! documentation was updated in all modified/added files".format(
                "\033[92m"
            )
        )
        sys.exit(0)
        ...
        print(
            ...
            "Files with missing or not updated DartDoc documentation found".format(
                "\033[91m"
            )
        )
        for failing_file in filtered_lookup:
            print(
                """>>> File name: {}\n\t{}\n""".format(
                    failing_file, filtered_lookup[failing_file]
                )
            )
        sys.exit(1)
